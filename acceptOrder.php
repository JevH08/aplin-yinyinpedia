        ...
        <div class="col-md-8 col-md-offset-2">
          <div class="block">
            <div class="product-list">
                <table class="table kurangiMargin">
                  <thead>
                    <tr>
                      <th class="">Buyer Name</th>
                      <th class="">Recipient's City</th>
                      <th class="">Recipient's Address</th>
                      <th class="">Recipient's Telephone</th>
                      <th class="">Product</th>
                      <th class="">Count</th>
                  ...
                  <tbody id="isiAcceptOrder">
                    <?php 
                    $sqlAmbilBuy = "SELECT d.no_detail, pem.nama_user, h.nama_tujuan, h.kota_tujuan , h.alamat_tujuan, h.telp_tujuan, p.nama_barang, d.jumlah, p.harga, j.nama_jasa, p.gambar FROM dtransaction d, htransaction h, produk_detail p, penjual pen, pembeli pem, jasa_pengiriman j WHERE d.id_barang = p.id_produk AND p.id_penjual = pen.id_penjual AND h.no_nota = d.no_nota AND h.id_user = pem.id_user AND j.id_jasa = h.id_pengiriman AND pen.id_penjual = $_SESSION[id] AND d.status = 'waiting'";
                    $queryAmbilBuy = mysqli_query($conn, $sqlAmbilBuy);
                    while($row = mysqli_fetch_array($queryAmbilBuy)){
						$idDetailBarang = $row['no_detail'];
						$gambarBarang = $row['gambar'];
                        $namaPembeli = $row['nama_user'];
                        $kotaPenerima = $row['kota_tujuan'];
                        $alamatPenerima = $row['alamat_tujuan'];
                        $teleponPenerima = $row['telp_tujuan'];
                        $namaBarang = $row['nama_barang'];
                        $jumlahBarang = $row['jumlah'];
                        $hargaBarang = $row['harga'];

                        echo '<tr id="row'.$idDetailBarang.'">
                            <td> '.$namaPembeli.' </td>
                            <td> '.$kotaPenerima.' </td>
                            <td> '.$alamatPenerima.' </td>
                            <td> '.$teleponPenerima.' </td>
                            <td> <img src="data:image;base64,'.base64_encode($gambarBarang).'" alt="image" width="80"/>' .$namaBarang.' </td>
                            <td> '.$jumlahBarang.' </td>
                            <td> '.$hargaBarang.' </td>
                            <td> '.$hargaBarang * $jumlahBarang.' </td>
                            <td>
                                <button class="remove btn btn-main btn-small btn-round-full  btn-icon" id="accept'.$idDetailBarang.'" onclick="accept('.$idDetailBarang.')">Accept Order</button>
                            </td>
                            <td>
                                <button class="remove btn btn-main btn-small btn-round-full  btn-icon" id="decline'.$idDetailBarang.'" onclick="decline('.$idDetailBarang.')">Decline Order</button>
                            </td>
                        </tr>';
                    }
                    ?>
...
<footer class="footer section text-center">
    ...
    <script type="text/javascript">
		function accept(idDetailBarang) {
    		alert(idDetailBarang);
    		$.get(
				"ajax.php",
				{
					mode:"acceptOrder",
		...
		function decline(idDetailBarang) {
    		alert(idDetailBarang);
    		$.get(
				"ajax.php",
				{
					mode:"declineOrder",
					idDetailBarang: idDetailBarang,
					jumlahBarang: <?php echo $jumlahBarang ?>
				},
				function (data) {
					$("#row" + idDetailBarang).remove();
				}
